...
void onInit(CBlob@ this)
{
	...
	this.push("names to activate", "angrykeg");
...
void AttackMovement(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
{
	...
	bool strong = (knight.swordTimer > KnightVars::slash_charge_level2/this.get_f32("COFFEE POWER"));//#TEST2
...
class SwordDrawnState : KnightState
{
	...
	bool TickState(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
	{
		...
		{
			const bool myplayer = this.isMyPlayer();
			if (knight.swordTimer == KnightVars::slash_charge_level2/this.get_f32("COFFEE POWER"))//#TEST2
			{
				Sound::Play("AnimeSword.ogg", pos, myplayer ? 1.3f : 0.7f);
				this.set_u8("animeSwordPlayed", 1);

			}
			else if (knight.swordTimer == KnightVars::slash_charge/this.get_f32("COFFEE POWER"))
			{
		...
		if (!this.isKeyPressed(key_action1))
		{
			if (delta < KnightVars::slash_charge/this.get_f32("COFFEE POWER"))
			{
				Vec2f vec;
				const int direction = this.getAimDirection(vec);
			...
			else if (delta < KnightVars::slash_charge_level2/this.get_f32("COFFEE POWER"))//#TEST2
			{
...
void SwordCursorUpdate(CBlob@ this, KnightInfo@ knight)
{
		...
		{
			int frame = 1 + int((float(knight.swordTimer - KnightVars::slash_charge/this.get_f32("COFFEE POWER")) / (KnightVars::slash_charge_level2 - KnightVars::slash_charge/this.get_f32("COFFEE POWER"))) * 9) * 2;
		...
		else if (knight.swordTimer < KnightVars::slash_charge/this.get_f32("COFFEE POWER") && knight.state == KnightStates::sword_drawn)
		{
			int frame = 2 + int((float(knight.swordTimer) / KnightVars::slash_charge/this.get_f32("COFFEE POWER")) * 8) * 2;
